...
import (
	"encoding/json"
	"fmt"
	config2 "github.com/sjmshsh/grpc-gin-admin/project_count_service/config"
...
var addr = config2.C.RabbitConfig.Addr[0]
...
// ConsumeTopicsCheckIn 消费签到的消息
func (r Rabbit) ConsumeTopicsCheckIn() {
	// 1.试探性创建交换机
	err := r.channel.ExchangeDeclare(
		r.Exchange, // 交换机名字
		"topic",    // 交换机类型，这里使用topic类型，即: Topics模式
...
func (r Rabbit) ConsumeFailedCache() {
	// 1.试探性创建交换机
	err := r.channel.ExchangeDeclare(
		r.Exchange, // 交换机名字
		"topic",    // 交换机类型，这里使用topic类型，即: Topics模式
	...
	go func() {
		for delivery := range msgs {
			...
			err = Rdb.Del(RCtx, key).Err()
			if err != nil {
				// 如果又失败了，就把消息保存在队列里面一直重试
				delivery.Ack(true)
			} else {
				// 成功了就消息确认
				delivery.Ack(false)
